...
* `spring-kafka-test` jar
    ** contains utilities -- for -- supporting testing
...
* built-in implementations
    ** `EmbeddedKafkaZKBroker`
        ...
        *** starts an embedded `Zookeeper` instance
            **** default | using `EmbeddedKafka`
...
* `org.springframework.kafka.test.utils.KafkaTestUtils`
    ** == static helper methods / allows
        *** consuming records,
        *** retrieving various record offsets
        *** setting up producer and consumer properties
        *** ...
* check https://docs.spring.io/spring-kafka/docs/current/api/org/springframework/kafka/test/utils/KafkaTestUtils.html[Javadocs]
* _Example1:_
[source, java]
----
/**
