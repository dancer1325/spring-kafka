    ...
    ** provides a
        *** high-level API -- for --
            **** serializing
                ***** record values
                ***** keys
                ***** / includes `org.apache.kafka.common.serialization.Serializer<T>` + implementations
                ***** -- can be specified via using -- `Producer` configuration properties
            **** deserializing
                ***** record values
                ***** keys
                ***** / includes the `org.apache.kafka.common.serialization.Deserializer<T>` + implementations
                ***** -- can be specified via using -- `Consumer` configuration properties

* _Example:_ ways to specify serializing & deserializing
...
* TODO: When a deserializer fails to deserialize a message, Spring has no way to handle the problem, because it occurs before the `poll()` returns.
